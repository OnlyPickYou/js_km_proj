...
_attribute_ram_code_ int	device_send_packet (u8 * p, u32 timeout, int retry, int pairing_link)
{
	...
	for (i=0; i<=retry; i += 1) {
		...
		ack_miss_no ++;
		if (ack_miss_no >= LL_CHANNEL_SEARCH_TH || paired_info.flag == KB_SEARCH_DONGLE_FLAG) {
			paired_info.flag &= (~KB_SEARCH_DONGLE_FLAG);
...
_attribute_ram_code_ int  rf_rx_process(u8 * p)
{
	...
	if (p_pkt->proto == RF_PROTO_BYTE) {
		...
		///////////// PIPE1: ACK /////////////////////////////
			...
			if(paired_info.flag & KB_SWITCH_DONGLE_FLAG){
				paired_info.num = paired_info.num & 0x03;
				paired_info.flag &= (~KB_SWITCH_DONGLE_FLAG);
			}
			rf_packet_ack_keyboard_t *k_pkt = (rf_packet_ack_keyboard_t *)(p + 8);

			pkt_km.num = (k_pkt->num & 0x03);
