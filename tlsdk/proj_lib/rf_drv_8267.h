...
#ifndef STANDARD_BLE_2M_MODE
#define STANDARD_BLE_2M_MODE	0  //TX and RX with TI ble 2m OK
#endif


#ifndef ACCESS_CODE_LEN
#define ACCESS_CODE_LEN			5
#endif
...
static inline void rf_set_tx_pipe (u8 pipe)
{
	...
	write_reg8 (0x800f15, 0x70 | pipe);  //f15<7>: 0 hw maintain sn/nesn disable
#else
	write_reg8 (0x800f15, 0xf0 | pipe);  //f15<7>: 1 hw maintain sn/nesn enable
#endif
}
...
#define		RF_LONG_PACKET_EN		0
	...
	#else
		#if (STANDARD_BLE_2M_MODE)  //ble 2m
/**************************************************************************************
  	  dma_len    hw_info  +  header +     payload  + CRC  + other  +  CRC_index
bytes: 	 4    +	    8     +    1(rf_len) +   x     +  3 	+    3         1
 	  p[0]= 16+x 				p[12] = x	 p[13]	 p[13+x]             p[16+x]
**************************************************************************************/
			#define		RF_PACKET_LENGTH_OK(p)		(p[0] == (p[12]&0x3f)+16)
			#define		RF_PACKET_CRC_OK(p)			((p[p[0]+3] & 0x51) == 0x40)
		#else  //private 2m
/**************************************************************************************
      dma_len    hw_info  +  header +     payload  + CRC  + other  +  CRC_index
bytes: 	 4    +	    8     +    1(rf_len) +   x     +  2 	+    3         1
 	  p[0]= 16+x 				p[12] = x	 p[13]	 p[13+x]             p[15+x]
**************************************************************************************/
