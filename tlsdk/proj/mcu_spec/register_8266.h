...
enum{
	FLD_ADC_REF_CHN0 = 	BIT_RNG(0,1),
	FLD_ADC_REF_CHN1 = 	BIT_RNG(2,3),
};

#define reg_adc_ch12_samp_res      REG_ADDR8(0x2f)
enum{
	FLD_ADC_CHN12_SAMP_RESOL = 	BIT_RNG(0,2),
};
...
enum{
	FLD_ADC_CTRL_AUTO_CHN1 = 	BIT(0),
	FLD_ADC_CTRL_AUD_OUTP = 	BIT(2),
	FLD_ADC_CTRL_AUTO_CHN0 =	BIT(3),
	FLD_ADC_CTRL_AUD_MODE = 	BIT_RNG(4,5),
	FLD_ADC_CTRL_DONE_SIG = 	BIT_RNG(6,7),
};
...
#define reg_adc_manu_start		REG_ADDR8(0x35)
enum{
	FLD_ADC_MANU_START = BIT(7),
};

#define reg_adc_chn2_outp		REG_ADDR16(0x36)
#define reg_adc_dat_byp_outp	REG_ADDR16(0x38)

#define reg_adc_status			REG_ADDR8(0x3a)
enum{
	FLD_ADC_BUSY_FLAG = 	BIT(0),
};


#define reg_adc_ch0_samp_clk_res	REG_ADDR8(0x3c)
enum{
	FLD_ADC_CHN0_SAMP_CYCLE = 	BIT_RNG(0,2),
	FLD_ADC_CHN0_SAMP_RESOL = 	BIT_RNG(3,5),
};

#define reg_adc_ch12_samp_clk   	REG_ADDR8(0x3d)
enum{
	FLD_ADC_CHN12_SAMP_CYCLE = 	BIT_RNG(0,2),
};
...
#define reg_gpio_wakeup_en		REG_ADDR8(0x5b5)
enum{
	FLD_GPIO_WAKEUP_EN		=	BIT(2),
	FLD_GPIO_IRQ_EN			=	BIT(3),
...
static inline	void config_dmic (int sample_rate) {		//16K configuration
	...
	reg_dmic_mod = 95 * 16000 / sample_rate;//188 * 16000 / sample_rate;					// dmic clock 192M (PLL) / 188 * 2 = 2M
