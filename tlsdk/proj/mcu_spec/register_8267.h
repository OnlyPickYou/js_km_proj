...
enum{
	FLD_CLK_SBC_EN =			0,  //just for compile pass
};
...
enum{
	FLD_IRQ_SBC_MAC_EN = 0,   //just for compile pass
};
...
static inline void config_adc (int chn_mic, int chn_adc, int sample) {
	...
	//reg_aud_hpf_alc = 11;				// reg0xb05 volume setting
	reg_aud_hpf_alc = 0x09;				// reg0xb05 be set as 0x09, last is 11
	reg_aud_alc_vol = 4;
	...
	if (sample == SYS_32M_AMIC_16K) {	//32M / (77 + 8 * 16 * 2) = 96K
		...
		//reg_dfifo_scale      = 0x24;  // down scale by 5
		//reg_adc_period_chn0  = 72;		//0x48 16M / (72 + 4 * 16 * 2) = 80K
		//reg_adc_period_chn12 = 4;
#if TL_MIC_32K_FIR_16K
	#if 1
        //96k(to 32k)   // no battery detect, white noise remain
        //reg_dfifo_scale       = 0x12;// reg0xb04[6:4] cic filter output select 1 , cic[22:5] , reg0xb04[3:0] down scale by 3,
        //reg_adc_period_chn0   = 103; // reg0x30 T1~4,  167(16M/96K) - 64 = 103 , 0x30[1:0] just for fine tuning

        //64k to 32k    // with battery detect, white noise remained
#if (CLOCK_SYS_CLOCK_HZ == 	48000000) //48M sysclk
		reg_dfifo_scale       = 0x22;
	...
	#else
        reg_dfifo_scale       = 0x11; // reg0xb04[6:4] cic filter output select 1 , cic[22:5] , reg0xb04[3:0] down scale by 2,
	    reg_adc_period_chn0   = 0x7a; // reg0x30 T1~4,  250(16M/64K) -128 = 122(72 /4 ) <<2 = 72 , 0x30[1:0] just for fine tuning
	    reg_adc_period_chn12  = 8;    // reg0x32 T5 = 8x16 = 128 , just T5 no T6, single channel
	    reg_adc_ctrl          = 0x15; // reg0x33[1] = 0 , disable T6 , mono mode[4]/L channel enable[3]/audio enable[1],  talk to zhidong.mao for detail
        reg_aud_alc_vol       = 0x20;//0x19; // 0xb06[5:0]: manual volume , corespond to  reg0xb04[6:4]
	#endif
#else

#if (CLOCK_SYS_CLOCK_HZ == 	48000000) //48M sysclk
		reg_dfifo_scale       = 0x45;
		reg_adc_period_chn0   = 260;
		reg_adc_period_chn12  = 15;
#else  //16M sysclk
        reg_dfifo_scale       = 0x44; // reg0xb04[6:4] cic filter output select 4 , cic[22:5] , reg0xb04[3:0] down scale by 5,
	    reg_adc_period_chn0   = 0x48; // reg0x30 T1~4,  200(16M/80K) - 128 = (122 /4 ) <<2 = 72 , 0x30[1:0] just for fine tuning
	    reg_adc_period_chn12  = 8;    // reg0x32 T5 = 8x16 = 128 , just T5 no T6, single channel
#endif
	    reg_adc_ctrl          = 0x15; // reg0x33[1] = 0 , disable T6 , mono mode[4]/L channel enable[3]/audio enable[1],  talk to zhidong.mao for detail
